...
function loadLogedInUserFriends() {
    ...
    $.ajax({
        ...
        success: function (friendListWithLogedInData) {
            if (friendListWithLogedInData != null)
                friendListWithLogedInData.forEach(printLogedInFriends);
        }
    });
}

function printLogedInFriends(friend) {
    var pair = friend;
    var userData = pair.left;
    var isLogedIn = pair.right;
    var userName = userData.fname + " " + userData.lname;
